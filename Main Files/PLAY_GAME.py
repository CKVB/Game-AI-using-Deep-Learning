...
def execute():
	...
	def apply_break():
		...
		time.sleep(1)
	def choice(data):
		Label(myapp,text="Click Stop to enable User mode",background="#117aee",foreground="#fff",font=font_style).place(relx=0.047, rely=0.313, height=61, width=584)	
		Label(myapp,text="Current choice : "+data,background="#117aee",foreground="#f9fd48",font=font_style).place(relx=0.047, rely=0.089, height=61, width=584)
		myapp.update()
	...
	while condition:		
		...
		if (prediction_1[3]>0.01 or prediction_2[3]>0.01) or forward_reverse_prediction[1]==1:
			...
			move_forward()
		elif(prediction_1[0]==1 or prediction_2[0]==1) and left_right_prediction[0]==1:
			move_left()
		elif(prediction_1[1]==1 or prediction_2[1]==1) and left_right_prediction[1]==1:
			move_right()
		else:
			choice("Neutral")
			keyboard.release('w')
		...
		if temp[-5:].count('w')==5:
			apply_break()
			temp=[]
def initial_introduction():
	Label(myapp,text="Game in user mode",background="#117aee",foreground="#fff",font=font_style).place(relx=0.047, rely=0.089, height=61, width=584)
	Label(myapp,text="Click Start to enable AI mode",background="#117aee",foreground="#fff",font=font_style).place(relx=0.047, rely=0.313, height=61, width=584)
	myapp.update()
def stop():
	global condition
	condition=False
...
initial_introduction()
Button(myapp,text="Start",command=execute,background="#174806",font=button_font,cursor="hand2",relief="groove",foreground="#00ffff").place(relx=0.188, rely=0.625, height=44, width=147)
Button(myapp,text="Stop",command=stop,background="#174806",font=button_font,cursor="hand2",relief="groove",foreground="#00ffff").place(relx=0.563, rely=0.625, height=44, width=147)
myapp.mainloop()